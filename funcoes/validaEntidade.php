	...
	if ($conv1 != $conv2) {
		...
		if ($totalRows_rsConv1 >= 1) { 
			...
			if ($row_rsConv1['ID_LINHA'] == 1) { 
				$CONV1 = "erro";
				$erro = "O Convenio N. ".$conv1." já estar cadastrado nos nossos registros."; 
			} else if ($row_rsConv1['ID_LINHA'] == 2) { 
				$CONV2 = "erro";
				$erro = "O Convenio N. ".$conv2." já estar cadastrado nos nossos registros."; 
			}
		} else { $valida2 = "ok"; }
	
	} else {
	
		$erro = "Os convênios devem ser diferentes.";
...
if ( (isset($_POST['Conta']) && isset($_POST['dvCC'])) && (isset($_POST['agencia']) && isset($_POST['dvAG'])) ) {
	...
	if ($CC != $AG) {
		...
		if ($totalRows_rsContaAgencia >= 1) { 
			$CONTA = "erro";
			$erro = "Entidade ".$row_rsContaAgencia['ID']." já cadastrada com mesmos números de agência e conta."; 
		} else { $valida3 = "ok"; }
	
	} else {
	
		$erro = "A Conta Corrente e a Agência devem ser diferentes.";
