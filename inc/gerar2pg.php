...
if ($pag2SDJ["$d_sd"] != "ok" && ($d_SDJ >= $d_i && $d_SDJ <= $d_f)) {
	...
	$query_rsGuias2RetSDJ = "SELECT id_guia, data_FEPJ_SDJ, tarifaFEPJ_SDJ FROM guias2fepj WHERE guias2fepj.data_FEPJ_SDJ = '$d_sd'";
	...
	if ($totalRows_rsGuias2RetSDJ > 0) {
		do {
			...
			$query_rsGuias2via = "SELECT guias.emicao, guias.dataRetornoFEPJ_SDJ, guias.dataRetornoFEPJ, guias.tipo, guias.idReg, guias.id, guias.id_entidade, guias.numero, guias.ano, guias.declarado, guias.tipoImovel, guias.id_natuEsc, entidades.farpen, entidades.convenio, entidades.id_sdt FROM guias
...
if ($row_rsGuias2via['numero'] < 10){ $numeros2 = "0000".$row_rsGuias2via['numero']; } else if ($row_rsGuias2via['numero'] < 100) { $numeros2 = "000".$row_rsGuias2via['numero']; } else if ($row_rsGuias2via['numero'] < 1000) { $numeros2 = "00".$row_rsGuias2via['numero']; } else if ($row_rsGuias2via['numero'] < 10000) { $numeros2 = "0".$row_rsGuias2via['numero']; } 
			...
			$anos2 = substr($row_rsGuias2via['ano'], 2, 2);
			
			$valorLinhaSDT['convenio'] = formatarExportacao ($row_rsEntidadeSDT['convenio'], "up", "7", "0");
			$valorLinhaSDT['numeroDocumento'] = $id_entidade2.$anos2.$numeros2; 
			...
			$valorLinhaSDT['compencacao'] = substr($row_rsGuias2RetSDJ['data_FEPJ_SDJ'], 8, 2).substr($row_rsGuias2RetSDJ['data_FEPJ_SDJ'], 5, 2).substr($row_rsGuias2RetSDJ['data_FEPJ_SDJ'], 0, 4);

			$linha2 = $valorLinhaSDT['convenio'].$valorLinhaSDT['numeroDocumento'].$valorLinhaSDT['farpen'].$valorLinha['docTipo'].$valorLinhaSDT['emissao'].$valorLinhaSDT['compencacao'].$valorLinha['vSDJ_FEPJ']."sdj2"; 
...
if ($pag2SNR["$d_fe"] != "ok" && ($d_SNR >= $d_i && $d_SNR <= $d_f)) {
	...
	mysql_select_db($database_Emolumentos, $Emolumentos);
	$query_rsGuias2RetSNR = "SELECT id_guia, data_FEPJ_Emolumento, tarifaFEPJ_SNR FROM guias2fepj WHERE guias2fepj.data_FEPJ_Emolumento = '$d_fe'";
...
//echo $d_fe;			
			...
			if ($row_rsGuias2via['tipo'] == "Registro") { 
				...
				mysql_select_db($database_Emolumentos, $Emolumentos);
				$query_rsEntidadeREGISTRAL = "SELECT entidades.id, entidades.farpen, entidades.convenio, entidades.convenio2, entidades.id_sdt FROM entidades WHERE entidades.id = '$id_ent'";
...
if ($row_rsGuias2via['numero'] < 10){ $numeros2 = "0000".$row_rsGuias2via['numero']; } else if ($row_rsGuias2via['numero'] < 100) { $numeros2 = "000".$row_rsGuias2via['numero']; } else if ($row_rsGuias2via['numero'] < 1000) { $numeros2 = "00".$row_rsGuias2via['numero']; } else if ($row_rsGuias2via['numero'] < 10000) { $numeros2 = "0".$row_rsGuias2via['numero']; } 
			...
			$v_FEPJ = $row_rsGuias2RetSNR['tarifaFEPJ_SNR'] * 100;
			$valorLinha['valorFEPJ'] = formatarExportacao ($v_FEPJ, "up", "8", "0");
			$valorLinha['numeroDocumento'] = $id_entidade2.$anos2.$numeros2; 
			...
			$valorLinha['compencacao'] = substr($row_rsGuias2RetSNR['data_FEPJ_Emolumento'], 8, 2).substr($row_rsGuias2RetSNR['data_FEPJ_Emolumento'], 5, 2).substr($row_rsGuias2RetSNR['data_FEPJ_Emolumento'], 0, 4);
			
			$linha = $valorLinha['convenio'].$valorLinha['numeroDocumento'].$valorLinha['farpen'].$valorLinha['docTipo'].$valorLinha['emissao'].$valorLinha['compencacao'].$valorLinha['valorFEPJ']."SN2";
			...
			echo "SNR - Guia pag dub: ".$row_rsGuias2RetSNR['id_guia']." Tarifa = R$".$v_FEPJ."<br>";
